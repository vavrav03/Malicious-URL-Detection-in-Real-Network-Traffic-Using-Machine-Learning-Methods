...
def fix_fp16_network(network_definition: trt.INetworkDefinition) -> trt.INetworkDefinition:
    ...
    :return: patched network definition
    """
    for idx in range(network_definition.num_layers - 1):
        layer = network_definition.get_layer(idx)
        next_layer = network_definition.get_layer(idx + 1)

        if (
            layer.type == trt.LayerType.ELEMENTWISE
            and hasattr(layer, "op")
            and layer.op == trt.ElementWiseOperation.POW
            ...
            next_layer.set_output_type(0, trt.DataType.FLOAT)
...
def _tensor_name(engine: trt.ICudaEngine, idx: int) -> str:
    """Return tensor/binding name at slot *idx* (works on all TRT versions)."""
    return engine.get_binding_name(idx) if hasattr(engine, "get_binding_name") else engine.get_tensor_name(idx)


def _is_input(engine: trt.ICudaEngine, idx: int) -> bool:
    """True if slot *idx* is an input tensor."""
    if hasattr(engine, "binding_is_input"):
        return engine.binding_is_input(idx)
    return engine.get_tensor_mode(_tensor_name(engine, idx)) == trt.TensorIOMode.INPUT


def _set_shape(context: trt.IExecutionContext, idx: int, shape):
    """Set dynamic shape for slot *idx* (old & new API)."""
    if hasattr(context, "set_binding_shape"):
        context.set_binding_shape(idx, shape)
    else:
        context.set_input_shape(_tensor_name(context.engine, idx), shape)


def _get_shape(context: trt.IExecutionContext, idx: int):
    """Return current tensor shape for slot *idx* (old & new API)."""
    if hasattr(context, "get_binding_shape"):
        return context.get_binding_shape(idx)
    return context.get_tensor_shape(_tensor_name(context.engine, idx))


def _execute_async(context: trt.IExecutionContext, bindings, stream_handle) -> bool:
    """
    Launch the network, covering every API variant:

    • execute_async_v2  (≤ 8.5)
    • execute_async     (≥ 8.6, 9, 10)
    • enqueue_async     (rare intermediate)
    • execute_v2        (sync fallback)
    """
    if hasattr(context, "execute_async_v2"):
        return context.execute_async_v2(bindings, stream_handle)

    if hasattr(context, "execute_async"):
        return context.execute_async(bindings=bindings, stream_handle=stream_handle)

    if hasattr(context, "enqueue_async"):
        return context.enqueue_async(bindings=bindings, stream_handle=stream_handle)

    # last-resort synchronous path (execute_v2 is always present)
    if hasattr(context, "execute_v2"):
        ok = context.execute_v2(bindings)
        # ensure compute is finished when we fake async
        torch.cuda.current_stream().synchronize()
        return ok

    raise AttributeError("No compatible TensorRT execute/enqueue method found.")
...
def get_output_tensors(
    ...
    """
    Reserve GPU memory for outputs and set dynamic shapes (TRT old & new APIs).
    """
    for host_tensor, slot in zip(host_inputs, input_binding_idxs):
        _set_shape(context, slot, tuple(host_tensor.shape))

    device_out: Dict[str, torch.Tensor] = {}
    for slot in output_binding_idxs:
        out_shape = _get_shape(context, slot)
        out_name = _tensor_name(context.engine, slot)
        device_out[out_name] = torch.empty(tuple(out_shape), device="cuda")
...
def infer_tensorrt(
    ...
    for slot in input_binding_idxs:
        name = _tensor_name(context.engine, slot)
        if name not in inputs:
            raise KeyError(f"missing input tensor: {name}")

        t = inputs[name]
        if t.dtype in (torch.int64, torch.long):
            logging.warning(f"{name}: casting {t.dtype} → int32 for TensorRT")
            t = t.to(torch.int32)
    ...
    out_tensors = get_output_tensors(context, in_tensors, input_binding_idxs, output_binding_idxs)

    bindings = [int(t.data_ptr()) for t in in_tensors + list(out_tensors.values())]

    stream = torch.cuda.current_stream().cuda_stream
    ok = _execute_async(context, bindings, stream)
    if not ok:
...
def get_binding_idxs(engine: trt.ICudaEngine, profile_index: int = 0):
    """
    Return lists of input- and output-binding indices for the selected profile.

    Compatible with both:
    - old API   - engine.num_bindings / engine.binding_is_input
    - new API   - engine.num_io_tensors / get_tensor_mode
    """
    ...
    if hasattr(engine, "num_bindings"):
        num_bindings_per_profile = engine.num_bindings // engine.num_optimization_profiles
        start = profile_index * num_bindings_per_profile
        end = start + num_bindings_per_profile
        for idx in range(start, end):
            (input_binding_idxs if engine.binding_is_input(idx) else output_binding_idxs).append(idx)
        return input_binding_idxs, output_binding_idxs
