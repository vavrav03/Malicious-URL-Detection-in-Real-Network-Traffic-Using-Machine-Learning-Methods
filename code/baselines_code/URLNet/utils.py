...
def read_data(file_path):
    df = pd.read_csv(file_path, usecols=["url", "label"])
    df = df.head(10000)
...
def get_word_vocab(urls, max_length_words, min_word_freq=0): 
    tokenizer = Tokenizer(lower=False, oov_token=None)  # You can customize this
    start = time.time()

    tokenizer.fit_on_texts(urls)
    # Filter out low-frequency words manually if needed
    if min_word_freq > 1:
        tokenizer.word_counts = {k: v for k, v in tokenizer.word_counts.items() if v >= min_word_freq}
        tokenizer.word_index = {k: i for i, (k, _) in enumerate(tokenizer.word_counts.items(), start=1)}
    ...
    return np.array(x), reverse_dict
...
def batch_iter(data, batch_size, num_epochs, shuffle=True):
    """
    Generates batches from a (possibly ragged) list of samples,
    without trying to cast them to a fixed-shape NumPy array.
    """
    data_list = list(data)
    ...
    for epoch in range(num_epochs):
        if shuffle:
            # shuffle by indices, then index into the list
            perm = np.random.permutation(data_size)
            shuffled = [data_list[i] for i in perm]
        else:
            shuffled = data_list
