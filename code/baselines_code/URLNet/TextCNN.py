    ...
    def __init__(self, char_ngram_vocab_size, word_ngram_vocab_size, char_vocab_size, \
        ...
        if mode == 3 or mode == 5: 
            with tf.name_scope("word_char_concat"): 
                ww = tf.get_variable("ww", shape=(num_filters_total, 512), initializer=tf.keras.initializers.glorot_uniform())
                ...
                wc = tf.get_variable("wc", shape=(num_filters_total, 512), initializer=tf.keras.initializers.glorot_uniform())
        ...
        with tf.name_scope("output"): 
            w0 = tf.get_variable("w0", shape=[1024, 512], initializer=tf.keras.initializers.glorot_uniform())
            ...
            w1 = tf.get_variable("w1", shape=[512, 256], initializer=tf.keras.initializers.glorot_uniform()) 
            b1 = tf.Variable(tf.constant(0.1, shape=[256]), name="b1") 
            l2_loss += tf.nn.l2_loss(w1) 
            l2_loss += tf.nn.l2_loss(b1) 
            output1 = tf.nn.relu(tf.matmul(output0, w1) + b1)
            
            w2 = tf.get_variable("w2", shape=[256,128], initializer=tf.keras.initializers.glorot_uniform())
            b2 = tf.Variable(tf.constant(0.1, shape=[128]), name="b2") 
            l2_loss += tf.nn.l2_loss(w2) 
            l2_loss += tf.nn.l2_loss(b2) 
            output2 = tf.nn.relu(tf.matmul(output1, w2) + b2) 
            
            w = tf.get_variable("w", shape=(128, 2), initializer=tf.keras.initializers.glorot_uniform()) 
